...
# ⚠️ 最重要: 日付フィルタリング
**全ての検索で、必ず time_range="week" パラメータを使用してください！**
これは必須要件です。time_range を設定しない検索は受け付けられません。

**検索実行方法**:
1. **必ず time_range="week" を設定**: 各検索クエリで time_range パラメータを "week" に設定
2. **古い記事を除外**: 公開日が {start_date} より前の記事は全て除外
3. **記事の公開日を確認**: 各記事について、必ずweb_fetchで本文を取得し、公開日を確認
4. **{start_date} より古い記事は破棄**: 公開日が過去7日間以内でない記事は、レポートに含めない
...
**必須: 全ての検索で time_range="week" を使用**
- 検索ツールを呼び出すたびに、必ず time_range="week" パラメータを含めてください
- 例: tavily_search(query="skill management", time_range="week")
- time_range を指定しない検索は無効です

**調査の進め方（Gemini API制限を考慮した最適化プラン）**:
1. まず基本検索ワードで8回程度検索し、主要トレンドを把握（全て time_range="week" 使用）
2. 技術・プラットフォーム関連で8回程度検索し、具体的なソリューションを調査（全て time_range="week" 使用）
3. 製造業特化で4回程度検索し、産業別の動向を確認（全て time_range="week" 使用）
4. 地域・業界トレンドで3回程度検索し、広い視野で情報を収集（全て time_range="week" 使用）
5. 不足している情報があれば、追加で2-3回検索して補完（全て time_range="week" 使用）
...
"""
    ...
    print("⚠️  重要: 全ての検索で time_range='week' を使用し、古い記事を除外")
