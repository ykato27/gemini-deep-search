...
def generate_report(target_year: int = None):
    ...
    search_tool = TavilySearch(
        ...
        # time_range="week",  # ❌ 初期化時に設定するとエージェントが変更できないため削除
        # エージェントがプロンプト指示に基づいて動的に time_range を設定できるようにする
    )
    ...
    year = target_year or today.year  # ← デフォルトは現在の年（参考用）
    print(f"📅 検索対象期間: {start_date} から {end_date} (過去7日間)")
...
# 🎯 タスク
**{start_date} から {end_date} までの過去7日間に公開された**スキルマネジメント・タレントマネジメント**に関する欧米の最新記事を調査し、詳細なレポートを作成してください。

# ⚠️ 最重要: 日付フィルタリング
**全ての検索で、必ず start_date="{start_date}" および end_date="{end_date}" パラメータを使用してください！**

TavilySearchツールは、start_dateとend_dateパラメータをサポートしています。
これらを使用することで、指定した日付範囲内に公開された記事のみを取得できます。

**検索実行方法（必須）**:
1. **必ずstart_dateとend_dateを設定**:
   - start_date="{start_date}"
   - end_date="{end_date}"
...
3. **禁止事項**:
   - start_dateやend_dateを指定しない検索は無効です
   - "{start_date}"より前に公開された記事は全て除外してください
   - 記事タイトルに「2025」が含まれていても、公開日が{start_date}より前なら除外

4. **公開日の確認**:
   - 各記事について、必ずweb_fetchで本文を取得
   - 記事のメタデータや本文から公開日を抽出
   - 公開日が不明な記事や、{start_date}より前の記事は除外
...
**必須: 全ての検索で start_date と end_date を使用**
- 検索ツールを呼び出すたびに、必ず start_date="{start_date}" と end_date="{end_date}" を含めてください
- 正しい例: tavily_search(query="skill management", start_date="{start_date}", end_date="{end_date}")
- 間違った例: tavily_search(query="skill management")  # ← start_date/end_dateがないのでNG
- これらのパラメータを指定しない検索は無効です

**調査の進め方（Gemini API制限を考慮した最適化プラン）**:
1. まず基本検索ワードで8回程度検索し、主要トレンドを把握（全てstart_date/end_date使用）
2. 技術・プラットフォーム関連で8回程度検索し、具体的なソリューションを調査（全てstart_date/end_date使用）
3. 製造業特化で4回程度検索し、産業別の動向を確認（全てstart_date/end_date使用）
4. 地域・業界トレンドで3回程度検索し、広い視野で情報を収集（全てstart_date/end_date使用）
5. 不足している情報があれば、追加で2-3回検索して補完（全てstart_date/end_date使用）

**公開日の厳格な確認**:
- 各記事をweb_fetchで取得後、公開日を確認
- 公開日が {start_date} より前の記事は、内容が良くても必ず除外
- レポートには、{start_date} から {end_date} の間に公開された記事のみを含める

合計20-25回の検索を通じて、**過去7日間に公開された記事のみ**を用いた包括的かつ深い調査レポートを作成してください。
（推定実行時間: 約5-6分、API呼び出し: 約50-60回 / 250回制限）
"""

    print(f"📊 調査対象期間: {start_date} ～ {end_date} (過去7日間)")
    print("🔍 スキルマネジメント・タレントマネジメントの最新動向調査を開始します...")
    print(f"⚠️  重要: 全ての検索で start_date='{start_date}' end_date='{end_date}' を指定")
